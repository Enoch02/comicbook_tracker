...
interface ComicDao {
    ...
    @Query("SELECT * FROM comic WHERE status LIKE 'completed'" +
            "ORDER BY CASE WHEN :order = 0 THEN title END ASC," +
            "CASE WHEN :order = 1 THEN title END DESC")
    ...
    @Query("SELECT * FROM comic WHERE status LIKE 'dropped'" +
            "ORDER BY CASE WHEN :order = 0 THEN title END ASC," +
            "CASE WHEN :order = 1 THEN title END DESC")
    fun getAllDroppedOrdered(order: Int): Flow<List<Comic>>

    @Query("SELECT * FROM comic WHERE status LIKE 'plan to read'" +
            "ORDER BY CASE WHEN :order = 0 THEN title END ASC," +
            "CASE WHEN :order = 1 THEN title END DESC")
